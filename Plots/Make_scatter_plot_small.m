function f = Make_scatter_plot_small(r,c,n_nodes,mode)
...
set(0, 'DefaultAxesTickLabelInterpreter', 'Latex')
    ...
    L1 = [1,2,3];
    for i = 1:length(L1)
        L3 =  [ L3; convertCharsToStrings(['$' num2str(i)  '$'])];
...
%                         convertCharsToStrings(['$' num2str(L1(i)) '_c$'])];
    ...
    f = figure('Renderer', 'painters', 'Position', [10 10 500 300]);
    subplot(2,1,1)
    ...
    ylabel('real part')
    legend({'analytical SC','EMTP'}) %'analytic senc coef','inverse jacob',
    set(gca,'FontSize',15) 
%     xticks(1:74)
%     xticklabels(n_L)
    set(gca,'FontSize',14) 
